...
export const requestUploadImage = async (data) => {
    const res = await apiClient.post(`/api/admin/products/upload-image`, data);
    return res.data;
};
...
export const requestUpdateProduct = async (id, data) => {
    const res = await apiClient.put(`/api/admin/products/update/${id}`, data);
    return res.data;
};

export const requestDeleteProduct = async (id) => {
    const res = await apiClient.delete(`/api/admin/products/delete/${id}`);
    return res.data;
};
...
export const requestSearchProduct = async (query) => {
    // Encode query để tránh lỗi với ký tự đặc biệt trong URL
    const encodedQuery = encodeURIComponent(query);
    const res = await request.get(`${apiProduct}/search/${encodedQuery}`);
    return res.data;
};
