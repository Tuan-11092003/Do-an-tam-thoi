...
function PaymentSucces() {
    ...
    // Sử dụng dữ liệu từ server thay vì tính toán lại
    const getSubtotal = () => {
        // Server đã tính subtotal cho mỗi item, chỉ cần tổng hợp
        return payment?.items?.reduce((sum, item) => sum + (item.subtotal || 0), 0) || 0;
    };
    ...
    const getTotal = () => {
        // Server đã tính finalPrice, sử dụng trực tiếp
        return payment?.finalPrice || getSubtotal() - getCouponDiscount();
    };
