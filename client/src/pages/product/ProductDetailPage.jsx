...
function ProductDetailPage() {
    ...
    const fetchProductById = async (options = {}) => {
        ...
        try {
            ...
            // Lấy dữ liệu đánh giá từ người dùng
        } catch (error) {
    ...
    const handleAddToCart = async () => {
        ...
        try {
            // ✅ Gọi API thêm vào giỏ hàng (backend tự động thêm vào đầu)
    ...
    const handleBuyNow = async () => {
        ...
        try {
            ...
            // Lấy itemId từ response để tick sản phẩm
            let finalAddedItemId = addCartRes.metadata?.addedItemId;
            
            // Nếu không có, lấy từ sản phẩm đầu tiên trong giỏ
            if (!finalAddedItemId && addCartRes.metadata?.cart?.products?.length > 0) {
                const firstProduct = addCartRes.metadata.cart.products[0];
                if (firstProduct && firstProduct._id) {
                    finalAddedItemId = firstProduct._id.toString();
                }
            }

            // Chỉ tick sản phẩm vừa thêm
            if (finalAddedItemId) {
