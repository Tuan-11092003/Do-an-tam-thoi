...
import { Minus, Plus, Trash2, ShoppingBag, CreditCard, Truck, Shield, Tag, X, Copy, Calendar, Home, ChevronRight, CheckCircle } from 'lucide-react';
...
function CartPage() {
    ...
    // ✅ Backend đã sắp xếp sẵn (sản phẩm mới nhất ở đầu)
    const orderedCartData = safeCartData;

    // Lấy danh sách mã giảm giá đang hoạt động từ server
    ...
    // Tải mã giảm giá khi component được mount
    useEffect(() => {
    ...
    const calculateSubtotal = (products) => {
        ...
        return products.reduce((sum, product) => {
            // Ưu tiên dùng subtotal từ server, nếu không có thì tính từ giá * số lượng
            return sum + (product.subtotal ?? (product.priceAfterDiscount ?? product.price) * product.quantity);
        }, 0);
    };

    // Tính giảm giá từ mã coupon (chỉ để xem trước, server sẽ tính chính xác khi áp dụng)
    const calculateCouponDiscount = (totalPrice, coupon) => {
        if (!coupon) return 0;
        return (totalPrice * coupon.discount) / 100;
    };

    // Tính tổng tiền cuối cùng (chỉ để xem trước)
    ...
    const handleRemoveItem = async (index) => {
        ...
        try {
            ...
            // ✅ TỐI ƯU: Chỉ cần reload giỏ hàng từ server
            await fetchCart();
                            ...
                            <div className="bg-white rounded-lg shadow-sm p-6">
                                ...
                                {!selectedCoupon ? (
                                    <div className="space-y-4">
                                        {/* Khuyến mãi dành cho bạn - UI giống trang chủ (Coupon) */}
                                                                ...
                                                                <div className="mt-3.5 pt-3 border-t border-gray-200">
                                                                    {coupon.isUsed ? (
                                                                        <button
                                                                            type="button"
                                                                            disabled
                                                                            className="w-full bg-green-500 text-white px-4 py-2 rounded-lg text-sm font-semibold cursor-not-allowed flex items-center justify-center space-x-2 shadow-sm"
                                                                        >
